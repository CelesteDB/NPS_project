...
cox7 <- coxph(Surv(time,status == 'retired') ~ Perigee + Apogee + strata(Period) + Mass + Users_Commercial + Users_Government + `Purpose_Space Science` + `Purpose_Technology Development` + dummy_data$Continent, data = dummy_data)

summary(cox7)
```
 All variables lose significance, except users_commercial. 


We proceed with a penalized cox model starting from the covariates chosen before the cox model (line 247) :
```{r}
...
coef
```
all coefficients different from zero --> let's go back to the previous model.
