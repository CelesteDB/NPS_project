...
Consider only American satellites:
```{r}
am <- which(data$Continent == "Americas")
data <- data[am,]
```
...
```{r}
selected <- c(17, 20, 35, 43)
idx <- which(surv.data$strata %in% selected)

surv.data.prova <- surv.data[idx,]
ggplot(surv.data.prova, aes(x = timeS, y = survival, color = factor(strata))) +
  geom_line(size = 1) +
  scale_color_discrete(name = "Satellites") +
...
launch_date <- data$`Launch Date`

# Calcola le date 
date_futuro_dirty <- as.Date(launch_date) + anni_interi * 365 
date_futuro <- na.omit(date_futuro_dirty)
...
anni <- seq(min(as.Date("2023-11-17")), by = "year", length.out = 22) #
anni <- year(anni)
...
# x11()
# Plot bidimensionale con il conteggio delle osservazioni senza 0
ggplot(dati_completi, aes(x = Mese, y = Anno, fill = Count)) +
  geom_tile(color = "black") +
  geom_text(aes(label = ifelse(Count == 0, NA, Count)), color = "black", size = 3) + # Aumenta la dimensione del testo
  scale_fill_gradient(low = "white", high = "blue", guide = "legend") +
  labs(x = "Month", y = "Year") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_y_continuous(breaks = unique(dati_completi$Anno)[seq(1, length(unique(dati_completi$Anno)), by = 2)])
guides(fill = guide_colorbar())
```

```{r}
#satelliti NA
sum(is.na(date_futuro_dirty))
#satelliti la cui data di manutenzione è troppo breve
length(which(date_futuro < as.Date("2023-11-17")))
#resto
dim(data)[1] - length(which(date_futuro < as.Date("2023-11-17"))) - sum(is.na(date_futuro_dirty))
```

Dates for selected satellites:
```{r}
Maintenance.Time <- quantiles[selected]
# (data_futuro_selcted <- as.Date("2023-11-17") + quantiles[selected]*365)
(launch_date <- data$`Launch Date`[selected])
(Maintenance.Date <-  as.Date(launch_date) + quantiles[selected]*365)

variables <- c(1,8,19,17,3,4,5)
cbind(data[selected, variables],Maintenance.Time,Maintenance.Date)
```
